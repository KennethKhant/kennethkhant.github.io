...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-white/10 bg-[#0b0c0f]/75 backdrop-blur">
      ...
      <div
        className={`md:hidden overflow-hidden border-b border-white/10 bg-[#0b0c0f]/95 backdrop-blur transition-[max-height,opacity,transform] duration-300 ${
          open
      ...
      {open && (
        <button
          ...
          onClick={() => setOpen(false)}
          className="fixed inset-0 top-14 bg-black/40 backdrop-blur-[2px] md:hidden"
        />
      )}
    </header>
  );
